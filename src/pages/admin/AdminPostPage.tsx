...
import { Plus, FileText, FileDown, ClipboardList, Calendar, CheckCircle, XCircle, Download, Users } from 'lucide-react'
...
function MouTab() {
  ...
  async function handleCreateTpl() {
    ...
    setSaving(true)
    try { await postService.createMouTemplate({ ...tplForm, description: tplForm.description || null }); toast('success', 'Template dibuat'); setTplModal(false); load() }
  ...
  return (
    <>
      <div className="flex justify-between items-center gap-3">
        <Button variant="outline" onClick={() => { setTplForm({ name: '', description: '', is_active: true }); setTplModal(true) }}><Plus className="w-4 h-4" /> Template</Button>
      ...
      <Modal isOpen={tplModal} onClose={() => setTplModal(false)} title="Tambah Template MOU"
        footer={<div className="flex gap-2 justify-end"><Button variant="outline" onClick={() => setTplModal(false)}>Batal</Button><Button loading={saving} onClick={handleCreateTpl}>Simpan</Button></div>}>
            ...
            <tbody>
              {letters.map((l: any) => (
                <tr key={l.id} className="border-b hover:bg-white/40">
                  <td className="px-5 py-3 font-mono text-xs">{l.document_number || '—'}</td>
                  <td className="px-5 py-3">{l.applicant?.registration_number || l.applicant_id}</td>
                  <td className="px-5 py-3 text-xs text-[var(--text-muted)]">{new Date(l.created_at).toLocaleDateString('id-ID')}</td>
                  <td className="px-5 py-3 text-right"><Button size="sm" variant="outline"><Download className="w-3 h-3" /> Download</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function MplsTab() {
  ...
  async function handleCreateSched() {
    if (!schedForm.title) return toast('warning', 'Judul wajib')
    setSaving(true)
    try { await postService.createMplsSchedule(schedForm); toast('success', 'Jadwal dibuat'); setSchedModal(false); load() }
    catch (e: any) { toast('error', e.message) } finally { setSaving(false) }
  }
  ...
  return (
    <>
      <div className="flex justify-between items-center gap-3">
        <Button variant="outline" onClick={() => { setSchedForm({ title: '', start_date: '', end_date: '', location: '', description: '' }); setSchedModal(true) }}><Plus className="w-4 h-4" /> Jadwal MPLS</Button>
      ...
      {assignments.length > 0 && (
        <div className="bg-white/60 backdrop-blur-sm rounded-2xl border border-[var(--border)] overflow-hidden mt-4">
          <table className="w-full text-sm">
            <thead className="bg-[var(--accent-subtle)] border-b">
              <tr><th className="text-left px-5 py-3 font-semibold">Peserta</th><th className="text-left px-5 py-3 font-semibold">Jadwal MPLS</th></tr>
            </thead>
            <tbody>
      ...
      <Modal isOpen={schedModal} onClose={() => setSchedModal(false)} title="Tambah Jadwal MPLS"
        footer={<div className="flex gap-2 justify-end"><Button variant="outline" onClick={() => setSchedModal(false)}>Batal</Button><Button loading={saving} onClick={handleCreateSched}>Simpan</Button></div>}>
        <div className="space-y-4">
